...
const BloodOxygenScreen = ({ navigation }) => {
  ...
  const [originalBloodOxygenData, setOriginalBloodOxygenData] = useState([]); 
  ...
  const Analysis = () => {
    if (originalBloodOxygenData.length > 0) { 
      const averageBloodPressure = calculateAverageBloodPressure(originalBloodOxygenData);
      const maxBloodPressure = Math.max(...originalBloodOxygenData);
      const minBloodPressure = Math.min(...originalBloodOxygenData);
      ...
      setBloodOxygenData(analysisData);
  ...
  const calculateAverageBloodPressure = (data) => { 
  ...
  const Count = () => {
    if (originalBloodOxygenData.length > 0) { 
      const highestData = Math.max(...originalBloodOxygenData);
      const lowestData = Math.min(...originalBloodOxygenData);
      const aboveNormalCount = originalBloodOxygenData.filter((data) => data > 95).length;
      const belowNormalCount = originalBloodOxygenData.filter((data) => data < 90).length;
  ...
  const fetchAllData = async () => {
    try {
      ...
      if (error) {
        ...
        if (data.length > 0) {
          const sensorData = data.map((item) => item.blood_oxygen);
          setBloodOxygenData(sensorData);
          setOriginalBloodOxygenData(sensorData); 
